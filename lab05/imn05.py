...
def ploter2(s1, s2, s3, s4, s5, k1, k2, k3, k4, k5, title):
    ...
    plt.plot(range(len(s2)), s2, 'b-', label="k="+str(k2))
    plt.plot(range(len(s3)), s3, 'k-', label="k="+str(k3))
    plt.plot(range(len(s4)), s4, 'g-', label="k="+str(k4))
    plt.plot(range(len(s5)), s5, 'y-', label="k="+str(k5))
